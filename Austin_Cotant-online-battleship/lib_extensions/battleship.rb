...
class BattleshipGame
  ...
  def play
    play_array = [@players[1], @players[-1]]
    play_array.each_with_index do |player, idx|
      set = false
      until set
        puts "Welcome #{player.name}"
        player.setup(play_array[idx - 1].board)
        if player.is_a? HumanPlayer
          puts "Are you happy with your set-up?"
          puts "(y/n)"
          input = gets.chomp
          if input == 'y'
            set = true
  ...
  def play_turn
    ...
    opp_ships = @boards[@active_player].count_ships #will make a count_ships method soon
    if human
    ...
    if human && opp_ships > @boards[@active_player].count_ships #will also make a count_ships method
