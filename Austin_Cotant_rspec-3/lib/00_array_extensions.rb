...
class Array
  def two_sum
    ...
    self.each_with_index do |el, i|
      answer += self.sum_to_0(el,i)
    end
    answer.uniq.sort
  end
  def sum_to_0(element, index)
    answer = []
    self.each_with_index do |el, i|
      if element + el == 0
        answer << [i, index].sort unless i == index
      end
    end
    answer
  end
...
class Array
  ...
  def my_transpose
    answer = Array.new(self.length) {Array.new(self[0].length)}
    self.each_index do |rowidx|
      self.each_index do |colidx|
        answer[rowidx][colidx] = self[colidx][rowidx]
      end
    end
    answer.map! {|el| el.compact}.delete([])
    answer
  end
end
