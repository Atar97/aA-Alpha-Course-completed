...
def array_sum_with_index(arr)
  answer = 0
  arr.each_with_index {|el , i| answer += el*i}
  answer
end
...
def price_is_right(bids, actual_retail_price)
  bids.reduce do |best, bid|
    if actual_retail_price - bid < actual_retail_price - best && actual_retail_price - bid > 0
      bid
    else
      best
    end
...
def num_factors(number)
  (1..number).reduce(0) do |counter, fact|
    if number % fact == 0
      counter +=1
    else counter
    end
...
def ordered_vowel_word?(word)
  vowels = ['a','e','i','o','u']
  vowels = word.chars.select {|letter| vowels.include?(letter.downcase)}
  vowels == vowels.sort
end
