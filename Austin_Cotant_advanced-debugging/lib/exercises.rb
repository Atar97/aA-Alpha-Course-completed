  ...
  def pair_sum(target)
    ...
    self.each_with_index do |el1, i|
      self.each_with_index do |el2, j|
        if el1 + el2 == target
          temp = [i,j].sort
          pairs << temp unless pairs.include?(temp) || i == j
        end
      end
    end
    pairs
...
def jumble_sort(str, alphabet = nil)
  alphabet = ('a'..'z').to_a unless alphabet
...
def transpose(matrix)
  result = Array.new(matrix[0].length, Array.new(matrix.length))
  result.map! {|el| el.dup}
